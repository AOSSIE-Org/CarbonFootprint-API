...
const getVehiclesReverseLookup = (emissions, relativeLocation) => new Promise((resolve, reject) => {
  ...
  let destinationCity; let destinationState; let distance; let newMileage; let
    noOfLitres;
    ...
    .then((val) => {
      ...
      do {
        ...
        if ((destinationCity.name !== val.city) && (newMileage > 10 && newMileage < 30)) {
          const geoDetailsDest = Helper.geodecodeFromLatLon(destinationCity.lat, destinationCity.lng);
          geoDetailsDest
            .then((details) => {
  ...
  results
    .then((val) => {
      ...
      for (let i = 1; i < val.length; i++) {
        ...
        const interDistance = Helper.getDistanceFromLatLon(sourceLocation.lat, sourceLocation.lng,
      ...
      if (matches.length > 1) {
        ...
        const railDistance = Helper.railDistanceInCoordinates(trainSourceLocation, trainDestLocation);
        railDistance
          .then((val) => {
            const newPassengerCount = Math.round((emissions.CO2) / (railcarDefault * val));
