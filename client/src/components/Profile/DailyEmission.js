...
export default class DailyEmission extends Component {
  componentDidMount() {
    this.create();
  }
  ...
  calendarHeatmap() {
    ...
    function chart() {
      ...
      let monthRange = ((d3.time && d3.time.months) || d3.timeMonths)(
        moment(yearAgo)
          .startOf("month")
          .toDate(),
        now
      ...
      <Responsive as={Segment} raised className="screenhide">
        <Header as="h3" style={{ display: "flex" }}>
          ...
          <Header.Content>
            Emissions Chart
            <Header.Subheader>
              Your CO2 emission for the past months
            </Header.Subheader>
          </Header.Content>
          <Button
            as={Link}
        ...
        <Grid centered>
          <Grid.Column>
            <div ref="jsHeatmap" style={{ margin: "-36px" }} />
          </Grid.Column>
        </Grid>
      </Responsive>
      // </div>
