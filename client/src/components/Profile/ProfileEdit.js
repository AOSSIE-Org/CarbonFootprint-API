...
export default class ProfileEdit extends Component {
  ...
  onEdit() {
    ...
    changedData.name =
      (this.state.MetaProfile.given_name || this.state.profile.given_name) +
      ' ' +
      (this.state.MetaProfile.family_name || this.state.profile.family_name);
    changedData.nickname = this.state.MetaProfile.nickname || this.state.profile.nickname;

    this.props.auth
      .updateData(this.state.profile.sub, { user_metadata: changedData })
      .then(data => {
        console.log(data);
            ...
            <Modal.Description>
              ...
              <Form>
                <Form.Group widths="equal">
                  <Form.Input
                    label="First name"
                    value={metaProfile.given_name || profile.given_name}
                    onChange={(e, { value }) => {
                      let MetaData = this.state.metaProfile;
                      MetaData.given_name = value;
                  ...
                  <Form.Input
                    label="Last name"
                    value={metaProfile.family_name || profile.family_name}
                    onChange={(e, { value }) => {
                      let MetaData = this.state.metaProfile;
                      MetaData.family_name = value;
                      this.handleChange(MetaData);
                    }}
                ...
                <Form.Group>
                  <Form.Input
                    label="Nickname"
                    value={metaProfile.nickname || profile.nickname}
                    onChange={(e, { value }) => {
                      let MetaData = this.state.metaProfile;
                ...
                <Form.TextArea
                  ...
                  onChange={(e, { value }) => {
                    ...
                    let Profile = this.state.profile,
                      MetaData = this.state.metaProfile;
